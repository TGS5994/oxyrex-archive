        ...
        if (goahead) {
            ...
            let reductionFactor = 1,
                ...
                accelerationFactor = (delt.length) ? (
                    (combinedRadius / 4) / (Math.floor(combinedRadius / delt.length) + 1)) : (0.001),
                depth = {
            ...
            if (doDamage) {
                ...
                if ((my.settings.isNecromancer || n.settings.isNecromancer) && Math.abs(my.shape) === Math.abs(n.shape)) {
                    if (my.settings.necroBullets || n.settings.necroBullets) necroTypes.push("bullet");
                    if (my.settings.necroDrones || n.settings.necroDrones) necroTypes.push("drone", "minion", "swarm");
